...
export default {
  ...
  methods: {
    async fetchMessages() {
      try {
        const response = await axios.get(`https://lostid.onrender.com/api/ids/${this.id}/messages/`);
    ...
    async sendMessage() {
      ...
      try {
        await axios.post(`https://lostid.onrender.com/api/ids/${this.id}/messages/post/`, {
